...
run_recovery_single <- function(mod,
  ...
  # --- Plot: recovery scatter with 95% intervals (teacher-style) ----
  ...
  ggsave("outputs/figs/figC_recovery_single.png", p, width = 11, height = 6, dpi = 200)
  ...
  ggsave("outputs/figs/figD_recovery_single_coverage.png", p_cov, width = 9, height = 4.8, dpi = 200)

  # --- Prior sensitivity (role of priors) on ONE representative dataset ----
  # We show how posteriors change when T is small vs large.
  # Keep it lightweight: pick one simulated dataset per T_extreme.
  for (T0 in c(min(Ts), max(Ts))) {
    ...
    ggsave(paste0("outputs/figs/figE_prior_sensitivity_T", T0, ".png"),
