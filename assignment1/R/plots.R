# R/plots.R
# -----------------------------------------------------------------------------
# Plotting helpers used by Assignment 1.
# -----------------------------------------------------------------------------

# Helper used by stat_summary(fun.data = ...): mean ± SE
mean_se <- function(x) {
  x <- x[is.finite(x)]
  m <- mean(x)
...
# For each pairing, compute mean payoff for A and bootstrap CI
...
# For each strategy, compute mean role advantage and bootstrap CI
...
# Fig 0: trial dynamics (mean ± SE cumulative payoff) + role-swap marker
plot_fig0_dynamics <- function(trials_df, out_file, swap_trial = 30) {
  ...
  p <- ggplot2::ggplot(sum_df, ggplot2::aes(x = trial, y = mean)) +
    ...
    ggplot2::labs(
      ...
      subtitle = "Mean cumulative payoff (±1 SE).\nDashed vertical line = role swap.",
      x = "Trial",
...
# Fig 1: tournament performance (mean payoff + bootstrap CI)
...
# Fig 4: payoff distributions (variance across simulations)
